...
def send_welcome(message):
    ...
    button2 = telebot.types.InlineKeyboardButton(text="[➖ 𝗠𝗔𝗶𝗡 𝗖𝗛𝗔𝗡𝗡𝗘𝗟 ➖]", url=CHANNEL_URL, style="danger")
    button3 = telebot.types.InlineKeyboardButton(text="[➖ | 𝗠 𝘅 𝗗™ 𝗔𝗟𝗟 𝗢𝗧𝗦  | ➖]", url=BOT_LIST, style="success")
    markup.add(button1)
    markup.add(button2)
    markup.add(button3)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("status") and "data" in data and "media" in data["data"]:
                ...
                bot.edit_message_text(
                    "‼ 𝗙𝗮𝗶𝗹𝗲𝗱 𝘁𝗼 𝗳𝗲𝘁𝗰𝗵 𝘃𝗶𝗱𝗲𝗼. 𝗔𝗣𝗜 𝗿𝗲𝘀𝗽𝗼𝗻𝘀𝗲 𝗶𝘀 𝗲𝗺pty 𝗼𝗿 𝗶𝗻𝘃𝗮𝗹𝗶𝗱.‼",
                    chat_id=message.chat.id,
                    message_id=processing_msg.message_id
                )
                return
            ...
            bot.edit_message_text(
                f"‼ 𝗙𝗮𝗶𝗹𝗲𝗱 𝘁𝗼 𝗳𝗲𝘁𝗰𝗵 𝘃𝗶𝗱𝗲𝗼. 𝗔𝗣𝗜 𝘃𝟐 𝗿𝗲𝘀𝗽𝗼𝗻𝗱𝗲𝗱 𝘄𝗶𝘁𝗵: {response.status_code}‼",
                chat_id=message.chat.id,
                message_id=processing_msg.message_id
            )
            return
        ...
        bot.edit_message_text(
            f"⚠️ 𝗙𝗮𝗶𝗹𝗲𝗱 𝘁𝗼 𝗰𝗼𝗻𝗻𝗲𝗰𝘁 𝘁𝗼 𝗦𝗲𝗿𝘃𝗲𝗿 ⚠️ : {str(e)}",
            chat_id=message.chat.id,
            message_id=processing_msg.message_id
        )
    except Exception as e:
        bot.edit_message_text(
            f"️ ⚠️ 𝗙𝗮𝗶𝗹𝗲𝗱 𝘁𝗼 𝗽𝗿𝗼𝗰𝗲𝘀𝘀 𝘆𝗼𝘂𝗿 𝗿𝗲𝗾𝘂𝗲𝘀𝘁 ⚠️  : {str(e)}",
            chat_id=message.chat.id,
            message_id=processing_msg.message_id
        )
